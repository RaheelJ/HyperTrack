classdef testTruthGenerator  < iTruthGenerator
    ...
    methods (Access = public)
        ...
        function this = testTruthGenerator()
            ...
            this.my_target_start_states = iTruthGenerator.create_target_struct(0);
...
%             addpath('Optimization'); 
            ...
            [Alt0,Lon0,Lat0,Altf,Lonf,Latf,err1]   = Read_Endpoints(config);
            [Alt1,Lon1,Lat1,Speed,n_wp,err2]       = Read_Waypoints(config);
            [Alt2,Lon2,Lat2,Radius,n_nfz,err3]     = Read_NFZ(config);
            if ((err1+err2+err3)>0)
                status=1;
                message='Initialization Error! (Inputs Parameters Violate Limits)';
                %disp(message);
                return;
            elseif ((err1+err2+err3) < 0)
                status=-1;
                message='Initialization Error! (Could not find the config file)';
                %disp(message);
                return;
            end
            
            %Initialize values of the target and store as the starting states
            this.my_targets(1).pos_x_or_lat = deg2rad(Lat0);
            this.my_targets(1).pos_y_or_long = deg2rad(Lon0);
            this.my_targets(1).pos_in_geo = 1;
            ...
            if (n_nfz>0)
                ...
                problem.constraints.gl(3:n_nfz+2) = deg2rad(Radius);
            end
        ...
        function [targets] = get_targets(this, current_time)
                if (this.my_status==0)
                    ...
                    for i=1:n_seg
                        if (current_time >= this.my_solution(i).t0 && current_time <= this.my_solution(i).tf)
                            this.my_targets.pos_x_or_lat = speval(this.my_solution(i),'X',3,current_time);
                            this.my_targets.pos_y_or_long = speval(this.my_solution(i),'X',2,current_time);
            ...
            %disp('Here is the target states');
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
