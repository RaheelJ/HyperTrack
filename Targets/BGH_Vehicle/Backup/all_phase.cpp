		 ...
		 adouble* xad, int iphase, Workspace* workspace)
{
	...
	*/
	...
	alpha=(iphase==1 || iphase==8)?states[8]:states[6];
			...
			int iphase, Workspace* workspace)
{
	...
	else if(iphase>=7)
	{
		...
		else if(iphase==8)
		{
			...
			e[5]=ee_1_f;
			e[6]=nn_f;
			e[7]=pow(ee_1_i, 2)+pow(ee_2_i, 2)+pow(ee_3_i, 2)+pow(nn_i, 2);
		}
	}
	//std::cout<<"In Integrand Cost Function"<<'\n'<<std::endl;
}
...
/*************************************************************************************************************************************************/	
	...
	for(i=1; i<=8; i++)
	{
		...
		problem.phases(i).nodes = (RowVectorXi(1) << 18).finished();
	}
	...
	problem.phases(7).nevents=1;
	problem.phases(8).nevents=8;

	
	//problem.phases(1).npath=1;
	//problem.phases(7).npath=2;
	//problem.phases(8).npath=2;
...
/*************************************************************************************************************************************************/	
	...
	double head_ang_min=-2*M_PI, head_ang_max=2*M_PI;
	...
	for (i=1; i<=8; i++)
	{
		if(i>=2 && i<=7)
		{
			problem.phases(i).bounds.lower.states << alt_min, lon_min, lat_min, speed_min, path_ang_min, head_ang_min, 0, bank_ang_min, m_min;
		...
		{
			problem.phases(i).bounds.lower.states << alt_min, lon_min, lat_min, speed_min, ee_min, ee_min, ee_min, nn_min, 0, m_min;
			problem.phases(i).bounds.upper.states << alt_max, lon_max, lat_max, speed_max, ee_max, ee_max, ee_max, nn_max, deg_to_rad(Param.alpha_max[i-1]), m_s3;
		}
		problem.phases(1).bounds.upper.states(9)=m0;
		problem.phases(2).bounds.upper.states(7)=m_s2;
...
/*************************************************************************************************************************************************/
	...
	problem.phases(7).bounds.lower.events << h_atm;
	problem.phases(7).bounds.upper.events << h_atm;
	
	problem.phases(8).bounds.lower.events << altf, deg_to_rad(lonf), deg_to_rad(latf), speedf, deg_to_rad(alphaf), ee_1f, nnf, 1;
	problem.phases(8).bounds.upper.events << altf, deg_to_rad(lonf), deg_to_rad(latf), speedf, deg_to_rad(alphaf), ee_1f, nnf, 1;
...
/*************************************************************************************************************************************************/	
	...
	for(i=1; i<=8; i++)
	{
		...
		else
		{
			...
			problem.phases(i).guess.states.row(3)=linspace(speed0, speedf, 12);
			problem.phases(i).guess.states.row(4)=linspace(1, 1, 12);
			problem.phases(i).guess.states.row(5)=linspace(M_PI/2, M_PI/2, 12);
			problem.phases(i).guess.states.row(6)=linspace(deg_to_rad(alpha0), deg_to_rad(25.0), 12);
			problem.phases(i).guess.states.row(7)=linspace(0, 0, 12);
			problem.phases(i).guess.states.row(8)=linspace(m_s2, m_s3, 12);
		}
		problem.phases(i).guess.controls = zeros(2,12);
		problem.phases(i).guess.controls.row(0) = 0.1*ones(1, 12);
		problem.phases(i).guess.controls.row(1) = 0.5*ones(1, 12);
	}
