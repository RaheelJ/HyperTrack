...
{	
	...
	smooth_linear_interpolation( &C_D0, mach, table_mach, table_C_D0, Param.n_mach);
	smooth_linear_interpolation( &C_L0, mach, table_mach, table_C_L0, Param.n_mach);
	smooth_linear_interpolation( &C_kk, mach, table_mach, table_C_kk, Param.n_mach);
...
/*************************************************************************************************************************************************/	
	...
	msdata.npath = 1;
	msdata.ninitial_events = 9;
	msdata.nfinal_events = 9;
	msdata.nodes = (Eigen::VectorXi(3) << 8, 22, 8).finished();				// nodes per segment
...
/*************************************************************************************************************************************************/	
	...
	//State Limits
	double h_min=0, h_max=282000;											//feet
	...
	//Time Limits
	double t0=0, t1=200, t2=200, t3=1800, t4=1800, t5=1800, tf=2000;
	...
	problem.phases(1).bounds.lower.controls << u_alpha_min, u_bank_min, u_mf_min;
	problem.phases(1).bounds.upper.controls << u_alpha_max, u_bank_max, u_mf_max;
	
	problem.phases(1).bounds.lower.path << throttle_min;
	problem.phases(1).bounds.upper.path << throttle_max;
	
	problem.phases(1).bounds.lower.StartTime = t0;
	problem.phases(1).bounds.upper.StartTime = t0;
	problem.phases(3).bounds.lower.EndTime = t5;
	problem.phases(3).bounds.upper.EndTime = tf;
...
/*************************************************************************************************************************************************/	
	...
	problem.phases(3).bounds.lower.events << 	hf, lonf, latf, Vf, path_angf, head_angf, m_min, alphaf, bank_angf;
	problem.phases(3).bounds.upper.events <<	hf, lonf, latf, Vf, path_angf, head_angf, m_max, alphaf, bank_angf;
					
	problem.bounds.lower.times.resize(1,4);
	problem.bounds.upper.times.resize(1,4);
	problem.bounds.lower.times << t0, t1, t3, t5;
	problem.bounds.upper.times << t0, t2, t4, tf;
...
/*************************************************************************************************************************************************/	
	...
	problem.phases(1).guess.states=zeros(9,5);
	problem.phases(1).guess.states.row(0)=linspace(h0, h0, 5);
	problem.phases(1).guess.states.row(1)=linspace(lon0, lon0, 5);
	problem.phases(1).guess.states.row(2)=linspace(lat0, lat0, 5);
	problem.phases(1).guess.states.row(3)=linspace(V0, V0, 5);
	problem.phases(1).guess.states.row(4)=linspace(path_ang0, path_ang0, 5);
	problem.phases(1).guess.states.row(5)=linspace(head_ang0, head_ang0, 5);
	problem.phases(1).guess.states.row(6)=linspace(m0, m0, 5);
	problem.phases(1).guess.states.row(7)=linspace(alpha0, alpha0, 5);
	problem.phases(1).guess.states.row(8)=linspace(bank_ang0, bank_ang0, 5);

	problem.phases(1).guess.controls = zeros(3,5);
	problem.phases(1).guess.controls.row(0) = 0.3*ones(1, 5);
	problem.phases(1).guess.controls.row(1) = 0*ones(1, 5);
	problem.phases(1).guess.controls.row(2) = u_mf_max*ones(1, 5);

	problem.phases(1).guess.time = linspace(t0, t1, 5);
	
	//Initial Guess Phase 2
	problem.phases(2).guess.states=zeros(9,5);
	problem.phases(2).guess.states.row(0)=linspace((h_min+h_max)/2, (h_min+h_max)/2, 5);
	problem.phases(2).guess.states.row(1)=linspace((lon0+lonf)/2, (lon0+lonf)/2, 5);
	problem.phases(2).guess.states.row(2)=linspace((lat0+latf)/2, (lat0+latf)/2, 5);
	problem.phases(2).guess.states.row(3)=linspace((V_min+V_max)/2, (V_min+V_max)/2, 5);
	problem.phases(2).guess.states.row(4)=linspace((path_ang_max+path_ang_min)/2, (path_ang_max+path_ang_min)/2, 5);
	problem.phases(2).guess.states.row(5)=linspace((head_ang0+head_angf)/2, (head_ang0+head_angf)/2, 5);
	problem.phases(2).guess.states.row(6)=linspace((m0+m_min)/2, (m0+m_min)/2, 5);
	problem.phases(2).guess.states.row(7)=linspace(alpha_max/2, alpha_max/2, 5);
	problem.phases(2).guess.states.row(8)=linspace(bank_ang0, bank_angf, 5);

	problem.phases(2).guess.controls = zeros(3,5);
	problem.phases(2).guess.controls.row(0) = 0.1*ones(1, 5);
	problem.phases(2).guess.controls.row(1) = 0*ones(1, 5);
	problem.phases(2).guess.controls.row(2) = (u_mf_min+u_mf_max)/2*ones(1, 5);

	problem.phases(2).guess.time = linspace(t2, t3, 5);
	
	//Initial Guess Phase 3
	problem.phases(3).guess.states=zeros(9,5);
	problem.phases(3).guess.states.row(0)=linspace(hf, hf, 5);
	problem.phases(3).guess.states.row(1)=linspace(lonf, lonf, 5);
	problem.phases(3).guess.states.row(2)=linspace(latf, latf, 5);
	problem.phases(3).guess.states.row(3)=linspace(Vf, Vf, 5);
	problem.phases(3).guess.states.row(4)=linspace(path_angf, path_angf, 5);
	problem.phases(3).guess.states.row(5)=linspace(head_angf, head_angf, 5);
	problem.phases(3).guess.states.row(6)=linspace(m_min, m_min, 5);
	problem.phases(3).guess.states.row(7)=linspace(alphaf, alphaf, 5);
	problem.phases(3).guess.states.row(8)=linspace(bank_angf, bank_angf, 5);

	problem.phases(3).guess.controls = zeros(3,5);
	problem.phases(3).guess.controls.row(0) = -0.3*ones(1, 5);
	problem.phases(3).guess.controls.row(1) = 0*ones(1, 5);
	problem.phases(3).guess.controls.row(2) = u_mf_min*ones(1, 5);

	problem.phases(3).guess.time = linspace(t4, tf, 5);
...
/*************************************************************************************************************************************************/	
	...
	MatrixXd x_ph1, x_ph2, x_ph3;
	MatrixXd t_ph1, t_ph2, t_ph3;
	MatrixXd u_ph1, u_ph2, u_ph3;
	MatrixXd x, u, t;
	...
	x.resize(10, x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	x.row(0) << x_ph1.row(0), x_ph2.row(0), x_ph3.row(0);
	x.row(1) << x_ph1.row(1), x_ph2.row(1), x_ph3.row(1);
	x.row(2) << x_ph1.row(2), x_ph2.row(2), x_ph3.row(2);
	x.row(3) << x_ph1.row(3), x_ph2.row(3), x_ph3.row(3);
	x.row(4) << x_ph1.row(4), x_ph2.row(4), x_ph3.row(4);
	x.row(5) << x_ph1.row(5), x_ph2.row(5), x_ph3.row(5);
	x.row(6) << x_ph1.row(6), x_ph2.row(6), x_ph3.row(6);
	x.row(7) << x_ph1.row(7), x_ph2.row(7), x_ph3.row(7);
	x.row(8) << x_ph1.row(8), x_ph2.row(8), x_ph3.row(8);
	...
	u.row(2)<<u_ph1.row(2), u_ph2.row(2), u_ph3.row(2);
...
/*************************************************************************************************************************************************/	
	...
	altitude = x.block(0,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	longitude = x.block(1,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols()); 
	latitude = x.block(2,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	speed_state = x.block(3,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
    path_angle = x.block(4,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	heading_angle = x.block(5,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	mass = x.block(6,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	attack_angle = x.block(7,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
	bank_angle = x.block(8,0,1,x_ph1.cols()+x_ph2.cols()+x_ph3.cols());
