...
void Coordinated_PNG::Interceptor::Reset() {
    ...
    LOS_dist_lat.resize(num_M);
    LOS_dist_lon.resize(num_M);
    LOS_ang_lat.resize(num_M);
    LOS_ang_lon.resize(num_M);
    lead_lat_m.resize(num_M);
    lead_lon_m.resize(num_M);
    phase.resize(num_M);
    x_dist.resize(num_M);
    y_dist.resize(num_M);
    energy.resize(num_M);
    ...
    lead_lat_t.resize(num_M);
    lead_lon_t.resize(num_M);

    // Measurement Data
    LOS_ang_lat_dot.resize(num_M);
    LOS_ang_lon_dot.resize(num_M);
    LOS_dist_lat_dot.resize(num_M);
    LOS_dist_lon_dot.resize(num_M);
    ...
    lead_S_lat_t.resize(num_M);
    lead_S_lon_t.resize(num_M);
    aT_S_lon.resize(num_M);
    aT_S_lat.resize(num_M);
    B_lat.resize(num_M);
    B_lon.resize(num_M);
...
bool Coordinated_PNG::Interceptor::Initialize(const string& XML_Parameters, string& message) {
    ...
    pugi::xml_node Algo = Root.child("CPNG");
    if (Root && Common && Algo) {}
    else {
    ...
    p = Read_Param(stod(Algo.child_value("p")), 0, 1, error);
    aT_max = Read_Param(stod(Algo.child_value("aT_max")), 2, 20, error);
    cov_ratio = Read_Param(stod(Algo.child_value("cov_ratio")), 0.34, 0.999, error);
    ...
    int n = 0;
    int i = 0;
    objectstate Temp_State{ 0 };
    for (pugi::xml_node Missile = Positions.first_child(); Missile; Missile = Missile.next_sibling())
    {
        if (Missile)
        {
            Temp_State.x_or_lat = Read_Param(Missile.attribute("Latitude").as_double(), -90, 90, error);
            Temp_State.y_or_lon = Read_Param(Missile.attribute("Longitude").as_double(), -180, 180, error);
            Temp_State.z_or_alt = Read_Param(Missile.attribute("Altitude").as_double(), 0, 6378100, error);
            Temp_State.id = -1;
            Temp_State.time = 0;
            Temp_State.state_in_geo = true;

            Initial_State.push_back(Temp_State);
        }
        n = n + 1;
    }
    ...
    for (i = 0; i < num_M; i++)
    {
        State.push_back(Initial_State[i]);
    }    
    Reset();
    return true;
};
void Coordinated_PNG::Interceptor::Reinitialize() {
    int i = 0;
    for (i = 0; i < num_M; i++)
    {
...
double Coordinated_PNG::Interceptor::sin_DOmF(int n)
{
    double A, B, C, Out;
    A = sin(n);
...
void Coordinated_PNG::Interceptor::Coop_Coverage(int n_max)
{
    double ACB;
    double scale_factor, lon, lat, num_M_;
    int i, test_num;
    ...
    {
    case 2:
        num_M_ = M_PI / asin(cov_ratio);
        num_M = round(num_M_);
        CoC.resize(num_M);
        for (i = 1; i <= num_M; i++)
        {
            scale_factor = cos(M_PI / num_M_) - sqrt(pow(cov_ratio, 2) - pow(sin(M_PI / num_M_), 2));
...
void Coordinated_PNG::Interceptor::Calc_Bias(int index)
{
    int i=index;

    B_lon[i] = -0.5 * CoC[i].lon * aT_max * cos(lead_lon_t[i]) * 9.8;
    B_lat[i] = -0.5 * CoC[i].lat * aT_max * cos(lead_lat_t[i]) * 9.8;

    //cout << "aT_S (Lon, Lat): " << CoC[i].lon << "\t" << CoC[i].lat << "\n";
    //cout << "Cooperative Biases (Lon, Lat): " << B_lon[i] << "\t" << B_lat[i] << "\n";
}
void Coordinated_PNG::Interceptor::Adjust_Bias(int index)
{
    int i = index;
    double Bbest_lon = B_lon[Find_Min(LOS_dist)];
    double Bbest_lat = B_lat[Find_Min(LOS_dist)];

    B_lon[i] = (1 - p) * B_lon[i] + p * Bbest_lon;
    B_lat[i] = (1 - p) * B_lat[i] + p * Bbest_lat;
}
void Coordinated_PNG::Interceptor::CPNG(int index) {
    int i=index;

    command_lat[i] = N * (abs(LOS_dist_lat_dot[i]) * LOS_ang_lat_dot[i] - B_lat[i]);
    command_lon[i] = 0;
    //command_lon[i] = N * (abs(LOS_dist_dot[i]) * LOS_ang_lon_dot[i] - B_lon[i]);
    //command_lon[i] = N * (abs(LOS_dist_dot[i]) * LOS_ang_lon_dot[i]);
    //if (cos(lead_lat_m[i]) != 0)
    //{
    //    command_lon[i] = command_lon[i] / cos(lead_lat_m[i]);
    //}
    
    Limit(command_lon[i], cov_ratio * aT_max * 9.8, -cov_ratio * aT_max * 9.8);
    Limit(command_lat[i], cov_ratio * aT_max * 9.8, -cov_ratio * aT_max * 9.8);

    Adjust_Bias(i);    
}
void Coordinated_PNG::Interceptor::Course::operator()(const state_t& x, state_t& dx, const double) {
...
void Coordinated_PNG::Interceptor::Check_Param(int index) {
    bool condition1 = false;
    condition1 = (speed_M < 0.000001 && phase[index] > 1) || (State[index].z_or_alt < 0) || (abs(State[index].x_or_lat) >= 89.999999);
    self_destruct = condition1;
}
void Coordinated_PNG::Interceptor::Track(double t_meas, int index) {
    int i, last_phase;
...
void Coordinated_PNG::Interceptor::First_Run(objectstate Target_State) {
    int i;
    ...
    for (i = 0; i < num_M; i++)
    {
        ...
        LOS_dist_lat[i] = LOS_dist[i] * sin(LOS_ang_lon[i]);
        lead_lat_m[i] = LOS_ang_lat[i] - deg_to_rad(head_ang[i]);
        lead_lon_m[i] = LOS_ang_lon[i] - deg_to_rad(path_ang[i]);

        lead_lat_t[i] = LOS_ang_lat[i] - head_ang_t;
        lead_lon_t[i] = LOS_ang_lon[i] - path_ang_t;
...
void Coordinated_PNG::Interceptor::Fine_Calculations(objectstate Last_Interceptor_State, objectstate Target_State, int index) {
    ...
    for (i = t1 + (t2 - t1) / fine_time_steps; i < (t2 - 0.000001); i = i + (t2 - t1) / fine_time_steps) {
        ...
        if (my_terminated)
        {
            best_j = Find_Min(LOS_dist_lat);
            /* Extract variables required to calculate metrics */
            if (LOS_dist_lat[best_j] < dist_LOS || dist_LOS < 0)
            {
                dist_LOS = LOS_dist_lat[best_j];
...
void Coordinated_PNG::Interceptor::Update_Target_State(objectstate Target_State)
{
    int i, last_phase, best_i;
    objectstate Last_Interceptor_State;
    double delta_q_lat, delta_q_lon, delta_R, delta_R_lon, delta_R_lat;
    double t_last, q_lat_last, q_lon_last, R_last, R_lat_last, R_lon_last;
    ...
    for (i = 0; i < num_M; i++)
    {
        ...
        Calc_LOS(State[i], Target_State, LOS_dist[i], LOS_ang_lat[i], LOS_ang_lon[i]);
        LOS_dist_lat[i] = LOS_dist[i] * cos(LOS_ang_lon[i]);
        LOS_dist_lon[i] = LOS_dist[i] * sin(LOS_ang_lon[i]);
        lead_lat_m[i] = LOS_ang_lat[i] - deg_to_rad(head_ang[i]);
        lead_lon_m[i] = LOS_ang_lon[i] - deg_to_rad(path_ang[i]);
        lead_lat_t[i] = LOS_ang_lat[i] - head_ang_t;
        lead_lon_t[i] = LOS_ang_lon[i] - path_ang_t;
        ...
        if (delta_t > 0.000001) {
            ...
            delta_R_lon = LOS_dist_lon[i] - R_lon_last;
            LOS_dist_dot[i] = delta_R / delta_t;
            LOS_dist_lat_dot[i] = delta_R_lat / delta_t;
            LOS_dist_lon_dot[i] = delta_R_lon / delta_t;
        }
    }
    Last_Target_State = Target_State;    
        
    best_i = Find_Min(LOS_dist_lat);
    /* Extract variables required to calculate metrics */
    if (LOS_dist_lat[best_i] < dist_LOS || dist_LOS < 0)
    {
        dist_LOS = LOS_dist_lat[best_i];
        dist_covered = energy[best_i];
        time_taken = State[best_i].time;
    }
    speed = speed_M;
    altitude = State[best_i].z_or_alt;
    accuracy = 1;
    num = num_M;
};

bool Coordinated_PNG::Interceptor::Get_State(objectstate& xo_state) {
    bool valid_state = Is_Started() && !my_killed;
    objectstate State_Out;

    int best_i = Find_Min(LOS_dist);
    State_Out = State[best_i];
    State_Out.y_or_lon = deg_to_long(State[best_i].y_or_lon);
    State_Out.x_or_lat = deg_to_lat(State[best_i].x_or_lat);
    if (valid_state)
    {
        xo_state = State_Out;
    }
...
void Coordinated_PNG::Interceptor::Get_Record() {
    int i;
    if (save_csv && valid_csv)
    {
        for (i = 0; i < num_M; i++)
        {
            Rec[i].csv("Output/CPNG_States_"+std::to_string(i), {"Time", "Latitude", "Longitude", "Altitude", "Heading_Angle", "Path_Angle", "LOS_Angle_Lat", "LOS_Angle_Lon", "Command_Lat", "Command_Lon"});
        }  
    }
}

bool Coordinated_PNG::Interceptor::Is_Started() {
    int best_i = Find_Min(LOS_dist);
    return (phase[best_i] > 0);
}
bool Coordinated_PNG::Interceptor::Is_Hit(int index) {
    return (LOS_dist_lat[index] < dist_stop);
}
bool Coordinated_PNG::Interceptor::Is_Missed(int last_phase, int index) {
    //return (phase[index] < last_phase);
    return 0;
}

bool Coordinated_PNG::Interceptor::Is_Terminated(bool& xo_interceptor, bool& xo_target, int& xo_target_id)
{
